...
import syn.cinocraft.config.OregenConfig;
    ...
    public void generateWorld(Random random, int chunkX, int chunkZ, World world, boolean newGen) {
        ...
        if (world.provider.getDimension() == DimensionType.OVERWORLD.getId()) {
            ...
            if (OregenConfig.GENERATE_NETHER) {
                addOreSpawn(ModBlocks.blockCinoOre, (byte) OreType.ORE_NETHER.ordinal(), Blocks.NETHERRACK, world, random, chunkX * 16, chunkZ * 16,
                        OregenConfig.MIN_VEIN_SIZE, OregenConfig.MAX_VEIN_SIZE, OregenConfig.CHANCES_TO_SPAWN, OregenConfig.MIN_Y, OregenConfig.MAX_Y);
            }
        } else if (world.provider.getDimension() == DimensionType.THE_END.getId()) {
            if (OregenConfig.GENERATE_END) {
                addOreSpawn(ModBlocks.blockCinoOre, (byte) OreType.ORE_END.ordinal(), Blocks.END_STONE, world, random, chunkX * 16, chunkZ * 16,
                        OregenConfig.MIN_VEIN_SIZE, OregenConfig.MAX_VEIN_SIZE, OregenConfig.CHANCES_TO_SPAWN, OregenConfig.MIN_Y, OregenConfig.MAX_Y);
            }
        }
